...
/*
 * Texture Data (Int, Byte, Float). [Not how the texture is stored but rather how the program should read the data].
 */
 ...
 * How to Use (Design):
 *	1) Request the appropriate texture to be created by specifying the type of texture you want and its string identifier
 *	2) Pass the ITexture pointer along with texture data/specification to the texture manager
			(TextureManager will interface with ITexture to call the appropriate OpenGL commands to allocate memory in VRAM)
 *	3) If a shader needs to use texture data, specify that the uniform is texture data and pass the identifier and the shader will
 *		grab the appropriate data that it needs from the TextureManager.
 *	4) Once no more mesh instances are using the texture and the texture becomes 'inactive,' the texture mananger will automatically
 *		delete it.
 * NOTE: In the off-chance that the user requests an ITexture but never loads data into it, the texture manager will recognize that will make it inactive and delete it.
...
public:
	...
	class ITexture* CreateTexture(const std::string& id, ETextureType, ETextureDataType);

	/*
	 * Set Texture Specification (information about the data that'll be coming in) and Data.
	 * Specifies: Number of textures (1 for single textures, n if this is an array, 6 for cube) and size of texture (width, height)
	 */
	void SetTextureSpecification(class ITexture*, int, int, int);

	/*
	 * Set Texture Data. Given the texture number (0 for a single texture), copy over the texture data (using memcpy).
	 */ 
